    ...
    Column(
        ...
        if (currencyTo == "" || currencyFrom == "") {
            ...
            Row(
                ...
                Text(
                    text = "-${
                        BigDecimal(currencyFromAmount).setScale(
                            5,
                            RoundingMode.HALF_EVEN
                        )
                    } ${vm.getCurrencySymbol(currencyFrom)}",
            ...
            Row(verticalAlignment = Alignment.CenterVertically,
                ...
                Column {
                    ...
                    Text(
                        text = "Баланс: ${vm.getCurrencySymbol(currencyTo)}${
                            BigDecimal(
                                balances[currencyTo] ?: 0.0
                ...
                Text(
                    text = "+${
                        BigDecimal(vm.amountTo).setScale(
                            5,
                            RoundingMode.HALF_EVEN
                        )
                    } ${vm.getCurrencySymbol(currencyTo)}",
                    textAlign = TextAlign.End
                )
            }
            ...
            // Обменный курс
            Text(
                text = "Обменный курс: 1 $currencyFrom = ${
                    BigDecimal(rate).setScale(
                        5,
                        RoundingMode.HALF_EVEN
                    )
                } $currencyTo",
            ...
            Button(
                onClick = {
                    ...
                    navController.popBackStack(
                        "currencies",
                        false
                    ) // Возвращаемся на экран "Валюты"
                },
                enabled = (balances[currencyFrom]
                    ?: 0.0) >= currencyFromAmount // Проверяем достаточно ли средств
            ) {
